...
const tabData = []
const  tab = ["userHuman", "userPet", "userXeno"]
...
function cardHuman(objet){
  ...
  para.textContent = obj.age + " ans - " + obj.email;
...
function markerProfil(profil, map) {
  const ICON = all.icon({
    iconUrl: profil.icon,
    iconSize: [50, 83],
    iconAnchor: [25, 83]
  });
  const marker = L.marker([profil.lat, profil.lng], { icon: ICON });
  marker.addTo(map);
}